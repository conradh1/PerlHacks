...
my $dsnname = "psadmin"; #assigns the name of the database
...
my $loginname= "postgres";
my $login_password= "password";
...
####################Start Program###########################################
     # $dbh = DBI->connect("dbi:Pg:dbname=$dbname", '', '', {AutoCommit => 0});
      ...
      $sth= $dbh->prepare("select * from pg_tables"); #prepare db query for executing
