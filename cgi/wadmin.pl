...
$basedir = "/usr/local/etc/httpd/htdocs/html/server/wwwboard/";
$baseurl = "http://www.athabascau.ca/html/server/wwwboard/";
$cgi_url = "http://www.athabascau.ca/cgi-bin/wadmin.pl";
...
#open(STUB, ">/usr/local/etc/httpd/htdocs/html/server/wwwboard/golf200/stat");
#print STUB "Startup.\n";
#close STUB;
...
#}
if ($ENV{'QUERY_STRING'} ne '') {
    ($confname, $command) = split(/&/, "$ENV{'QUERY_STRING'}");
    $temp = $confname;
    ($handle, $confname) = split(/=/, $confname);
    if ($handle ne "confname") {
	$confname = $temp;
    }
}
else {
  ...
  foreach $pair (@pairs)
  {
    ...
    # multiple field instances should be put into a list
    if (!$FORM{$name})
    {
      $FORM{$name} = $value;
    }
    ...
    #print "DEBUG: $name set to [$FORM{$name}]<BR>";
  }

  # Check and format the REAL conference name. Remove all invalid characters
  # and convert to lowercase.
  $FORM{'cname'} =~ tr/A-Z/a-z/;
  $FORM{'cname'} =~ s/[^\w\.]//g;
  $FORM{'csubname'} =~ tr/A-Z/a-z/;
  $FORM{'csubname'} =~ s/[^\w\.]//g;
  $confname = $FORM{'cname'};
  $confname .= "-$FORM{'csubname'}" if $FORM{'csubname'};
  $confname = $FORM{'confname'} if $FORM{'confname'};
#  &error(no_conference)  unless $confname;
}
...
if ($command eq 'remove') {
   ...
   print "[ <a href=\"$cgi_url\?$confname&remove\">Remove</a> ] [ <a href=\"$cgi_url\?$confname&remove_by_date\">Remove by Date</a> ] [ <a href=\"$cgi_url\?$confname&remove_by_author\">Remove by Author</a> ] [ <a href=\"$cgi_url\?$confname&remove_by_num\">Remove by Message Number</a> ] [ <a href=\"$baseurl/$mesgpage\">$title</a> ]\n";
   print "</font></center><hr size=7 width=75%><p>\n";
   print "<form method=POST action=\"$cgi_url\">\n";
   print "<input type=hidden name=\"action\" value=\"remove\">\n";
   print "<input type=hidden name=\"confname\" value=\"$confname\">\n";
   print "<table border>\n";
   print "<tr>\n";
   print "<th colspan=6>Admin Password: <input type=password name=\"password\"></th>\n";
   print "</tr><tr>\n";
   ...
   foreach (@ENTRIES) {
      print "<tr>\n";
      print "<th><b>$_</b> </th><th><input type=radio name=\"$_\" value=\"all\"> </th><th><input type=radio name=\"$_\" value=\"single\"> </th><td><a href=\"$baseurl/$mesgdir/$_\.$ext\">  $SUBJECT{$_} </a></td><td>  $AUTHOR{$_} </td><td>$DATE{$_}<br></td>\n";
      print "</tr>\n";
   }
   print "</table>\n";
   print "<center><p>\n";
...
###########################################################################
   ...
   print "<table border>\n";
   print "<tr>\n";
   print "<th colspan=6> Admin Password: <input type=password name=\"password\"><br></th>\n";
   print "</tr>\n";
   print "<tr>\n";
   print "<th>Post # </th><th>Thread </th><th>Single </th><th>Subject </th><th> Author</th><th> Date</th></tr>\n";
...
###########################################################################
   ...
   print "<table border>\n";
   print "<tr>\n";
   print "<th colspan=4>Admin Password: <input type=password name=\"password\"><br></th>\n";
   print "</tr>\n";
   print "<tr>\n";
...
###########################################################################
   ...
   print "<ul>\n";
   print "<li><a href=\"$cgi_url\?$confname&remove\">Remove Files</a>\n";
   print "<li><a href=\"$cgi_url\?$confname&remove_by_num\">Remove Files by Mesage Number</a>\n";
   print "<li><a href=\"$cgi_url\?$confname&remove_by_date\">Remove Files by Date</a>\n";
   print "<li><a href=\"$cgi_url\?$confname&remove_by_author\">Remove Files by Author</a>\n";
   print "</ul><br>\n";
   print "<form  action=\"$cgi_url\?$confname\">\n";
   print "Conference Name: <input name=\"confname\" size=25 maxlength=25>\n";
...
sub error {
   ...
   if ($error eq 'bad_combo') {
      print "<html><head><title>Bad Admin Password </title></head>\n";
      print "<body><center><h1>Bad Admin Password</h1></center>\n";
      print "You entered an invalid password.  Please try again.<p>\n";
      &passwd_trailer
   }
   ...
   elsif ($error eq 'no_conference') {
      print "<html><head><title>No Conference Selected</title></head>\n";
      print "<body><center><h1>No Conference Selected</h1></center>\n";
      print "Cannot proceed until a Conference has been selected.<p>\n";
      &passwd_trailer
   }
